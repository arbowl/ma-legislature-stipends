...
def generate_session_report(loaded: LoadedSession) -> SessionReport:
    ...
    validation_summary = {
        'catalog_errors': len([
            i for i in catalog_issues if str(i.level) == 'ERROR'
        ]),
        'catalog_warnings': len([
            i for i in catalog_issues if str(i.level) == 'WARNING'
        ]),
        'session_errors': len([
            i for i in session_issues if str(i.level) == 'ERROR'
        ]),
...
def _generate_summary_stats(
        ...
        if chamber_results:
            ...
            by_chamber[chamber] = {
                ...
                'median_compensation': median([
                    r['total'] for r in chamber_results
                ])
            }
    by_party = {}
    ...
    stipend_distribution = {
        ...
        '1_or_more_stipends': len([
            r for r in results if r['stipends_9b'] > 0
        ])
    }
