...
def test_scrape_leadership_count_2025_2026(raw_leadership_2025_2026):
    ...
    senate_count = sum(
        1 for entry in raw_leadership_2025_2026 if entry["chamber"] == "senate"
    )
    house_count = sum(
        1 for entry in raw_leadership_2025_2026 if entry["chamber"] == "house"
    )
...
def test_scrape_leadership_senate_positions(raw_leadership_2025_2026):
    ...
    president = next(
        (e for e in senate_leaders if "President of the Senate" in e["raw_title"]), None
    )
    majority_leader = next(
        (e for e in senate_leaders if e["raw_title"] == "Majority Leader"), None
    )
    minority_leader = next(
        (e for e in senate_leaders if e["raw_title"] == "Minority Leader"), None
    )
...
def test_scrape_leadership_house_positions(raw_leadership_2025_2026):
    ...
    speaker = next(
        (e for e in house_leaders if "Speaker of the House" in e["raw_title"]), None
    )
    majority_leader = next(
        (e for e in house_leaders if e["raw_title"] == "Majority Leader"), None
    )
    minority_leader = next(
        (e for e in house_leaders if e["raw_title"] == "Minority Leader"), None
    )
...
def test_normalize_case_insensitive():
    ...
    assert (
        normalize_leadership_title("PRESIDENT OF THE SENATE", "senate")
        == "SENATE_PRESIDENT"
    )
    assert (
        normalize_leadership_title("president of the senate", "senate")
...
def test_normalize_with_extra_whitespace():
    ...
    assert (
        normalize_leadership_title("  Majority Leader  ", "senate")
        == "SENATE_MAJORITY_FLOOR_LEADER"
    )
...
def test_e2e_all_leadership_members_exist(
    raw_leadership_2025_2026, session_members_2025_2026
):
    """Verify every leadership member exists in the members file"""
    member_ids_in_session = {
        m["member_id"] for m in session_members_2025_2026["members"]
    }
    for entry in raw_leadership_2025_2026:
        member_id = entry["member_id"]
        assert (
            member_id in member_ids_in_session
        ), f"Leadership member {member_id} ({entry['raw_title']}) not found in members.json"


def test_e2e_mapped_leadership_roles_in_session(
    raw_leadership_2025_2026, session_roles_2025_2026
):
...
def test_e2e_specific_members_2025_2026(session_roles_2025_2026):
    ...
    for member_id, expected_role, name in test_cases:
        ...
        assert (
            expected_role in member_roles[member_id]
...
def test_e2e_division_chairs_2025_2026(session_roles_2025_2026):
    ...
    for member_id, expected_role, name in division_chairs:
        assert (
            member_id in member_roles
        ), f"Division Chair {name} ({member_id}) not found"
        assert (
            expected_role in member_roles[member_id]
...
def test_no_duplicate_exact_titles_per_member(raw_leadership_2025_2026):
    ...
    for entry in raw_leadership_2025_2026:
        ...
        assert (
            raw_title not in member_titles[member_id]
...
def test_senate_has_exactly_one_president(raw_leadership_2025_2026):
    ...
    presidents = [
        e
        for e in raw_leadership_2025_2026
